...
class Model(ModelDesc):
    ...
    def _build_graph(self, input_vars):
        ...
        def new_get_variable(name, shape=None, **kwargs):
            ...
            if name != 'W' or 'conv0' in v.op.name or 'linear' in v.op.name:
                return v
            else:
        ...
        def residual(name, l, increase_dim=False, first=False):
            ...
            with tf.variable_scope(name) as scope:
                ...
                c1 = activate(c1) #fg(c1)
                c2 = Conv2D('conv2', c1, out_channel)
                c2 = activate(c2) #fg(c2)
                if increase_dim:
        ...
        wd_w = tf.train.exponential_decay(0.0002, get_global_step_var(),
                                          480000, 0.2, True)
        wd_cost = tf.mul(wd_w, regularize_cost('.*/W', tf.nn.l2_loss), name='wd_cost') #regex = linear.*/W ??
