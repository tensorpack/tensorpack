...
class GraphRewriter(object):
    ...
    def check_conv_block(self, current_conv_node):
        is_conv_block_zer0 = False
        # For FasterRCNNFPN the first block alone is flipped for resnet
        if FLAGS.model_name in ["FasterRCNNFPN"]:
            nodenamesplit = current_conv_node.name.split("/")
            if len(nodenamesplit) > 3:
                convstring = nodenamesplit[3]
                groupstring = nodenamesplit[0][:5]
                blockstring = nodenamesplit[1][:6]
                convopstring = nodenamesplit[2][:4]
                if convstring == "Conv2D" and \
                    groupstring == "group" and \
                        blockstring == "block0" and \
                            convopstring == "conv":
                    is_conv_block_zer0 = True    
        return is_conv_block_zer0
    ...
    def intel_cpu_eightbitize_nodes_recursively(self, current_node):
        ...
        if len(nodenamesplit) > 1:
            ...
            blockstring = nodenamesplit[1][:5]
            if groupstring == "group0" and blockstring == "block":
                print("push: {}".format(current_node.name))
        ...
        if len(nodenamesplit) > 1:
            ...
            blockstring = nodenamesplit[1][:5]
            if groupstring == "group0" and blockstring == "block":
                print("POP: {}".format(current_node.name))
          ...
          and (current_node.name not in self.excluded_nodes):
            ...
            if parent[0].op == "BiasAdd" and \
                ...
                if len(nodenamesplit) > 1:
                    ...
                    blockstring = nodenamesplit[1][:5]
                    if groupstring == "group0" and blockstring == "block":
                        #print("fusion: {} with parent {} grandparent {}".format(current_node.name, parent[0].op, grand_parent[0].op))
                        #print("fusion: grand_grand_parent : {}".format(grand_grand_parent))
                        print("current:Conv, parent: BiasAdd, GP:Add, GGP:Relu")
                        print("fusion: self.state.output_node_stack[-3][3] : {}".format(self.state.output_node_stack[-3][3]))
                        print("fusion: self.state.output_node_stack[-4][3] : {}".format(self.state.output_node_stack[-4][3]))
                ...
                if grand_grand_parent[0].op in ("Relu", "Relu6") \
                    ...
                    if len(nodenamesplit) > 1:
                        ...
                        blockstring = nodenamesplit[1][:5]
                        if groupstring == "group0" and blockstring == "block":
                            print("Fused: {}".format(current_node.name))
                    is_RN50_block_0 = self.check_conv_block(current_node)
                    self.state.output_node_stack[-2][3] = True  # BiasAdd to be fused
