...
class GraphRewriter(object):
    ...
    def remove_duplicate_nodes(self, graph):
        """Builds a mapping of node names to their defs from the graph."""
        nodes_map = {}
        for node in graph.node:
            #if node.name == 'rpn/box/b':
            #    print("entered")
            if node.name not in nodes_map.keys():
                nodes_map[node.name] = node
            else:                
                #if node.name in ['rpn/box/b', 'rpn/class/b']:
    ...
    def rewrite(self, output_node_names):
        ...
        if self.mode == "round":
            ...
            #dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/nirooptensorpack/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
            #tf.train.write_graph(self.input_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def0.pb", False)
            #tf.train.write_graph(self.input_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def0.pbtxt", True)
            dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/nirooptensorpack/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
            ...
            #dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/nirooptensorpack/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
            #tf.train.write_graph(self.output_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def1.pb", False)
            #tf.train.write_graph(self.output_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def1.pbtxt", True)
    ...
    def intel_cpu_eightbitize_nodes_recursively(self, current_node):
        ...
        if current_node.op == "Conv2D":
            if FLAGS.model_name not in ["FasterRCNN", "R-FCN", "FasterRCNNFPN"]:
                ...
                if FLAGS.model_name not in ["FasterRCNNFPN"]:
    ...
    def remove_redundant_quantization(self, old_graph):
        ...
        Raises:
          ValueError: Two nodes with the same name were found in the graph.
        """
        old_graph = self.remove_duplicate_nodes(old_graph)
        old_nodes_map = self.create_nodes_map(old_graph)
        #dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/nirooptensorpack/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
        #tf.train.write_graph(old_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def3.pb", False)
        #tf.train.write_graph(old_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def3.pbtxt", True)
        self.output_graph = graph_pb2.GraphDef()
        #self.output_graph = old_graph
        inputs_to_rename = {}
        ...
        #dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/nirooptensorpack/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
        #tf.train.write_graph(old_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def4.pb", False)
        #tf.train.write_graph(old_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def4.pbtxt", True)
        # Finally we apply all the rewiring we've marked to the graph.
        for node in old_graph.node:
            length = len(old_graph.node)
    ...
    def remove_dead_nodes(self, output_names):
        ...
        #dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/nirooptensorpack/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
        #tf.train.write_graph(old_output_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def5.pb", False)
        #tf.train.write_graph(old_output_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def5.pbtxt", True)        
