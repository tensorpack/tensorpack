...
class GraphRewriter(object):
    ...
    def rewrite(self, output_node_names):
        ...
        if self.mode == "round":
            ...
            if FLAGS.strip_redundant_quantization:
                ...
                dir_path = "/localdisk/niroop/repo/fasterrcnnfpn/tensorpack/examples/FasterRCNN/temp/built_graph/debug/"
                tf.train.write_graph(self.output_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def0.pb", False)
                #tf.train.write_graph(self.output_graph, dir_path, "Fasterrcnnfpn_inter_int8_graph_def0.pbtxt", True)
    ...
    def model_and_node_specific_action(self, current_node, get_add_node=False):
        ...
        if FLAGS.model_name in ["FasterRCNN", "R-FCN"]:
            ...
            if len(nodenamesplit) > 3:
                ...
                groupstring = nodenamesplit[0][:6]
                blockstring = nodenamesplit[1][:6]
                convopstring = nodenamesplit[2]
                if convstring == "Conv2D" and groupstring == "group0" and blockstring == "block0":
                    should_quantize_conv = True
            if len(nodenamesplit) > 2:
                groupstring = nodenamesplit[0][:6]
                blockstring = nodenamesplit[1][:6]
                addstring = nodenamesplit[2]
                if addstring == "add" and groupstring == "group0" and blockstring == "block0":
                    is_add_node = True

        elif FLAGS.model_name in ["RESNET50"]:
            nodenamesplit = current_node.name.split("/")
            if len(nodenamesplit) > 4:
                convstring = nodenamesplit[4]
                groupstring = nodenamesplit[1][:10]
                blockstring = nodenamesplit[2][:5]
                if convstring == "Conv2D" and groupstring == "resnet_v10" and blockstring == "conv4":
                    should_quantize_conv = True
        else:
            should_quantize_conv = False
    ...
    def identify_add_nodes(self, current_node):
        shortcut_conv = False
        if FLAGS.model_name in ["FasterRCNNFPN"]:
            nodenamesplit = current_node.name.split("/")
            if len(nodenamesplit) == 4:
                convopstring = nodenamesplit[3]            
                if convopstring == "add":
    ...
    def intel_cpu_eightbitize_nodes_recursively(self, current_node):
        ...
        if len(nodenamesplit) > 1:
            groupstring = nodenamesplit[0][:6]
            blockstring = nodenamesplit[1][:6]
            if groupstring == "group0" and blockstring == "block0":
        ...
        if current_node.op == "ConcatV2":
            should_quantize_concat = FLAGS.model_name not in [
                "FasterRCNN", "R-FCN", "FasterRCNNFPN"] and not ('map/while' in current_node.name)
        ...
        if current_node.op in ("AddN", "Add"):
                #is_RN50_add_node = self.model_and_node_specific_action(current_node, True)
                #if is_RN50_add_node:
                #    print("ok rn")
                #else:
                inputs = reversed(inputs)
        ...
        if len(nodenamesplit) > 1:
            ...
            blockstring = nodenamesplit[1][:6]
            if groupstring == "group0" and blockstring == "block0":
                print("Fusion: {}".format(current_node.name))
          ...
          and (current_node.name not in self.excluded_nodes):
            ...
            if parent[0].op == "BiasAdd" and \
                ...
                if len(nodenamesplit) > 1:
                    ...
                    blockstring = nodenamesplit[1][:6]
                    if groupstring == "group0" and blockstring == "block0":
                        print("BiasAdd: {} with parent {} grandparent {}".format(current_node.name, parent[0].op, grand_parent[0].op))
                        print("BiasAdd: grand_grand_parent : {}".format(grand_grand_parent))
                        print("BiasAdd: self.state.output_node_stack[-3][3] : {}".format(self.state.output_node_stack[-3][3]))
                        print("BiasAdd: self.state.output_node_stack[-4][3] : {}".format(self.state.output_node_stack[-4][3]))
                ...
                if grand_grand_parent[0].op in ("Relu", "Relu6") \
                    ...
                    if len(nodenamesplit) > 1:
                        ...
                        blockstring = nodenamesplit[1][:6]
                        if groupstring == "group0" and blockstring == "block0":
                            print("srini: {}".format(current_node.name))
                    #is_RN50_add_node = self.model_and_node_specific_action(grand_parent[0], True)
                    self.state.output_node_stack[-2][3] = True  # BiasAdd to be fused
                    self.state.output_node_stack[-3][3] = True  # AddN to be fused
                    self.state.output_node_stack[-4][3] = True  # Relu to be fused
                    bias_node_name = node_name_from_input(parent[0].input[1])
                    bias_node = self.nodes_map[bias_node_name]
                    #if not is_RN50_add_node:
                    add_node_name = node_name_from_input(grand_parent[0].input[0])
                    #else:
                    #    add_node_name = node_name_from_input(grand_parent[0].input[1])
