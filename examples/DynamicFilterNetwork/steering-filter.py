...
Bert De Brabandere et al.

This also demonstrated `monitors.put_image`.
"""


@layer_register()
def dynamic_conv_filter(inputs, filters, out_channel,
                        kernel_shape,
                        stride=1,
                        padding='SAME'):
    ...
    Args:
        ...
        filters : learned filters (dynamically generated by the network).
...
class ThetaImages(ProxyDataFlow, RNGDataFlow):
    ...
    def get_filter(theta, sigma=1., filter_size=9):
        ...
        used_filter = np.matmul(g.T, gp)
        used_filter = np.cos(theta) * used_filter + np.sin(theta) * used_filter.T
    ...
    def filter_with_theta(image, theta, sigma=1., filter_size=9):
        ...
        The filter parameters are returned to the user in the structure H.
    ...
    def get_data(self):
        for image in self.ds.get_data():
            theta = self.rng.uniform(0, 2 * np.pi)
            filtered_image, used_filter = ThetaImages.filter_with_theta(image[0], theta)
            yield [theta, image[0], filtered_image, used_filter]
